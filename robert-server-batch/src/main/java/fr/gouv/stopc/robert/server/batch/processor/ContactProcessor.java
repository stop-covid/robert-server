...
@Slf4j
@AllArgsConstructor
public class ContactProcessor implements ItemProcessor<Contact, Registration> {
	...
	@Override
	public Registration process(Contact contact) throws RobertServerCryptoException, RobertScoringException {
		log.info("Contact processing started");

		// Step 1 (parsing) has already been executed

		if (!step2CheckCountryCodeManagedByServer(contact)) {
			return null;
		}
		...
		if (!step4CheckIfidAIsARegisteredUser(contact, idA, registration)) {
			return null;
		}
	...
	private boolean step2CheckCountryCodeManagedByServer(Contact contact) {
		...
		if (!Arrays.equals(serverCountryCode, countryCode)) {
			log.info("Country code {} is not managed by this server ({}); rerouting contact to federation network",
	...
	private boolean step5CheckDeltaTaAndTimeABelowThreshold(HelloMessageDetail helloMessageDetail) {
		...
		if (Math.abs(timeFromHelloNTPsecAs16bits - timeFromDeviceAs16bits) > timeDiffTolerance) {
			log.warn("Time tolerance was exceeded: |{} (HELLO) vs {} (receiving device)| > {}; discarding HELLO message",
					 timeFromHelloNTPsecAs16bits, timeFromDeviceAs16bits, timeDiffTolerance);
			return false;
		}
		return true;
	}
	...
	private boolean step6CheckTimeACorrespondsToEpochiA(byte[] epochId, int timeFromDevice) {
		...
		long epochIdFromMessage = TimeUtils.getNumberOfEpochsBetween(tpstStartNTPsec,
																	 Integer.toUnsignedLong(timeFromDevice));
		...
		if (Math.abs(epochIdFromMessage - epochIdFromEBID) > 1) {
			log.warn("Epochid from message {}  vs epochid from ebid  {} > 1 (tolerance); discarding HELLO message",
					 epochIdFromMessage, epochIdFromEBID);
			return false;
		}
		return true;
	}
	 ...
	 */
	private boolean step8VerifyMACIsValid(byte[] ebid, byte[] encryptedCodeCountry,
			HelloMessageDetail helloMessageDetail, Registration registration) {
		...
		MacHelloValidationRequest request = MacHelloValidationRequest.newBuilder()
																	 ...
																	 .setDataToValidate(ByteString.copyFrom(ByteUtils.addAll(part1,
																															 part2)))
																	 .build();
		boolean validMAC = this.cryptoServerClient.validateMacHello(request);
	 ...
	 */
	private List<EpochExposition> step9AddContactInListOfExposedEpochs(Contact contact, byte[] epochId,
	...
	private List<EpochExposition> getExposedEpochsWithoutEpochsOlderThanContagiousPeriod(
			List<EpochExposition> exposedEpochs, int epochIdFromEBID) {
		// Purge exposed epochs list from epochs older than contagious period (C_T)
		return CollectionUtils.isEmpty(exposedEpochs) ? new ArrayList<>() : exposedEpochs.stream().filter(epoch -> {
			// TODO: Externaliser cette valeur.
	...
	private boolean validateContactHelloMessages(Contact contact, Registration registration, byte[] epochId) {
		...
		return contact.getMessageDetails()
					  .stream()
					  .map(item -> validateHelloMessage(contact.getEbid(), contact.getEcc(), item, registration,
														epochId))
					  .allMatch(item -> item == true);
	}

	private boolean validateHelloMessage(byte[] ebid, byte[] encryptedCodeCountry,
			HelloMessageDetail helloMessageDetail, Registration registration, byte[] epochId) {
		final long timeFromDevice = helloMessageDetail.getTimeCollectedOnDevice();
	...
	private class EncryptedEBID {
		...
		public EncryptedEBID decrypt() {
			...
			DecryptEBIDRequest decryptEbidRequest = DecryptEBIDRequest.newBuilder()
																	  .setEbid(ByteString.copyFrom(this.contact.getEbid()))
																	  .build();
			...
			this.epochId = new byte[4];
			this.idA = new byte[5];
			System.arraycopy(decryptedEBID, 0, this.epochId, 1, this.epochId.length - 1);
			System.arraycopy(decryptedEBID, this.epochId.length - 1, this.idA, 0, this.idA.length);
			return this;
		}
	}
