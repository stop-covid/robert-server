...
import fr.gouv.stopc.robertserver.crypto.RobertServerCryptoApplication;
...
class CryptoServiceImplTest {
    ...
    @Test
    void assertLengthTest() {
        int valueInt1 = 123;
        byte[] valueByte1 = ByteUtils.intToBytes(valueInt1);
        System.out.println(String.format("String valueInt1 length : %s-bits", valueByte1.length * 8));
        Exception e = null;
        try {
            this.cryptoService.assertLength("valueInt1", 32, ByteUtils.intToBytes(valueInt1));
        } catch (Exception ex) {
            e = ex;
        }
        assertNull(e);
